...
<template>
    <div class="tasks__sort">
        ...
        <ul class="sort__list">
            <li class="sort__item" :class="{'--sort-descending': sortBy[key] === ESortType.descending, '--sort-ascending' : sortBy[key] === ESortType.ascending}" v-for="(value, key) in sortBy" :key="key" @click="changeSortBy(key, value)">{{ EEngRuSortedNames[key] }}</li>
        </ul>
    </div>
</template>
